...
RSpec.describe Expenses::InstallmentCreator do
  ...
  describe '.call' do
    it 'creates N unpaid installments with split amounts and spaced dates' do
      ...
      expect(result.expenses.map.with_index { |e, i| [e.installment_number, e.installment_count] }.uniq).to eq([[1, 3], [2, 3], [3, 3]])
    end

    it 'spaces weekly installments' do
      result = described_class.call(base, { period: 'weekly', repetitions: 3 })
      dates = result.expenses.map(&:date)
      expect(dates.first + 2.weeks).to eq(dates[2])
    end

    it 'spaces biweekly installments' do
      result = described_class.call(base, { period: 'biweekly', repetitions: 3 })
      dates = result.expenses.map(&:date)
    ...
    it 'spaces annual installments from the first due date' do
      result = described_class.call(base, { period: 'annual', repetitions: 2 })
    ...
    it 'returns failure when repetitions are insufficient' do
      result = described_class.call(base, { period: 'monthly', repetitions: 1 })

      expect(result.success?).to be(false)
      expect(result.expense.errors[:base]).to be_present
    end

    it 'returns failure when create! raises RecordInvalid' do
      allow(Expense).to receive(:transaction).and_yield
      invalid = Expense.new
      invalid.errors.add(:base, 'falhou teste')
      allow(Expense).to receive(:create!).and_raise(ActiveRecord::RecordInvalid.new(invalid))

      result = described_class.call(base, { period: 'monthly', repetitions: 2 })
