...
RSpec.describe Dashboard::ProfitSeriesService do
  describe '#monthly' do
    it 'returns a 12-month profit series for the year' do
      ...
      result = described_class.new(year: 2025, month: nil).monthly
  ...
  describe '#daily' do
    it 'returns nil when no month is provided' do
      expect(described_class.new(year: 2025, month: nil).daily).to be_nil
    end
    ...
    it 'returns a daily profit series for each day of the month' do
      ...
      result = described_class.new(year: 2025, month: 1).daily
