...
RSpec.describe BottomNavComponent, type: :component do
  ...
  context 'when active: :home' do
    ...
    it 'renders 6 tabs with Lucide icons' do
      expect(html.scan('<a ').size).to eq(6)
    ...
    it 'dims inactive tabs with text-slate-400' do
      expect(html.scan('text-slate-400').size).to eq(5)
    end
  ...
  context 'when active: :goals' do
    let(:active) { :goals }

    it 'highlights only the goals tab' do
