...
RSpec.describe StatCardComponent, type: :component do
  describe '#view_template' do
    ...
    it 'uses the default value size when value_size is not provided' do
      html = view_context.render(described_class.new(title: 'T', value: 'V', color: :green, icon: nil))

      expect(html).to include('text-xl')
    end

    it 'applies a custom value_size class when provided' do
      html = view_context.render(described_class.new(title: 'T', value: 'V', color: :green, icon: nil, value_size: 'text-sm'))
