module Vehicles
  class TankBalanceCardComponent < ApplicationComponent
    def initialize(balance:, full:, last_fill:, variant: :mobile)
      ...
      @last_fill = last_fill
      @variant = variant
      @status = Vehicles::TankStatus.for(balance, full)
    end
    ...
    def danger?
      %i[empty negative].include?(@status.key)
    end
    ...
    def card_classes
      border = if danger?
                 ...
                 @status.key == :low ? 'border-amber-200' : 'border-slate-100'
      end
    ...
    def header_row
      div(class: 'flex items-center justify-between mb-2') do
        ...
        span(class: "text-[10px] font-bold uppercase tracking-wide rounded-full px-2 py-1 border #{@status.chip_class}") do
          t("vehicle.tank.status.#{@status.key}")
        end
      end
    end
    ...
    def balance_row
      div(class: 'flex items-end justify-between gap-3') do
        div(class: 'min-w-0') do
          p(class: "text-3xl font-bold tracking-tight tabular-nums #{@status.num_class}") { balance_text }
    ...
    def progress_bar
      div(class: 'h-2 bg-slate-100 rounded-full overflow-hidden mt-3') do
        div(class: "h-full rounded-full #{@status.bar_class}", style: "width: #{remain_pct}%")
      end
    end
    ...
    def note_line
      if @status.key == :ok
        ...
        p(class: "text-xs mt-2.5 #{@status.num_class}") { t("vehicle.tank.note.#{@status.key}") }
      end
    end
